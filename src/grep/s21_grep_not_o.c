...
void cleanMemory(int numOfpatterns, char** patterns, char** fileForF, int PatternFiles);
...
int main(int argc, char** argv) {
    ...
    if(opt.e_flag == 0 && opt.f_flag == 0) {
        if(patternWithoutE (argc, argv, patterns))
            numOfpatterns = 1;
    }
        ...
        if(argv[i][k] == 'e') {
            ...
            if(i == (argc - 1) && argv[i][k + 1] == '\0') {
                *err = 2;
                ...
                patternEorFileF(argv, i, k, numOfpatterns, patterns);
            }
        }
        else if(argv[i][k] == 'i')
            opt->i_flag = 1;
            ...
            if(i == (argc - 1) && argv[i][k + 1] == '\0') {
                *err = 2;
                ...
                patternEorFileF(argv, i, k, PatternFiles, fileForF);
...
void patternEorFileF(char** argv, int i, int k, int* l, char** str) {  // парсим шаблон после флага -е или название файла после флага -f
...
void copyEOstr (char *dest, char *src, int k) {  // копирует src в dest начиная с k-го элемента
    int m = 0;
    while(src[k] != '\0') {
        dest[m] = src[k];
        m++;
